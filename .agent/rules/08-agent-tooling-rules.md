...
Two more Claude Code plugins are installed at project scope — `ponytail` and `agent-skills`. Their
rules, and the tie-break against `superpowers` when both cover the same job, live in
