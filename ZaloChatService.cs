...
namespace AutoJMS
{
    public class ZaloChatService
    {
        ...
        public async Task ProcessReminders()
        {
            try
            {
                if (string.IsNullOrWhiteSpace(_appsScriptUrl))
                    return;
        ...
        public async Task<bool> SendZaloMessage(string message)
        {
            ...
            try
            {
                ...
                // 2. KỊCH BẢN JS GỘP - DÙNG CLIPBOARD ĐỂ BYPASS REACT
                string script = $@"
        (async function() {{
            try {{
                var ten = {tenNVJson};
                // Khởi tạo chuỗi mã đơn có khoảng trắng ở đầu để thoát thẻ Tag xanh
                ...
                // BƯỚC 1: Xóa sạch nội dung cũ
                document.execCommand('selectAll', false, null);
                document.execCommand('delete', false, null);

                // BƯỚC 2: Gõ @Tên và kích hoạt popup
                document.execCommand('insertText', false, '@' + ten);
                input.dispatchEvent(new Event('input', {{ bubbles: true }}));
                
                // Chờ 800ms cho popup tìm kiếm hiện lên
                await new Promise(r => setTimeout(r, 800));

                // BƯỚC 3: Click chọn người dùng trong Popup
                // BƯỚC 3: Chọn người dùng trong Popup (Sử dụng MouseEvent để trị React)
                    ...
                    if (items.length > 0) {{
                        ...
                        for (var i = 0; i < items.length; i++) {{
                            ...
                            if (nameEl && nameEl.innerText.toLowerCase().includes(target)) {{
                                // Phải dùng mousedown thì React mới ghi nhận thao tác chọn
                        ...
                        // FALLBACK: Nếu không khớp chữ, tự động ấn chọn gợi ý ĐẦU TIÊN
                        if (!isClicked) {{
                ...
                // Chờ 500ms để thẻ Tag màu xanh được tạo xong
                await new Promise(r => setTimeout(r, 500));

                // BƯỚC 4: DÁN MÃ ĐƠN BẰNG CLIPBOARD EVENT (GIẢ LẬP CTRL+V)
                // Đây là cách duy nhất an toàn để Zalo lưu nội dung vào State
input.focus();
                
                // Ép con trỏ chuột xuống vị trí tuyệt đối cuối cùng (sau thẻ Tag)
                ...
                // Nhịp 1: Chèn 1 dấu cách để thoát thẻ Tag, rồi xuống dòng (\n)
...
// Cực kỳ quan trọng: Ép React lưu toàn bộ (Tag + Xuống dòng + Mã) vào State
                input.dispatchEvent(new Event('input', {{ bubbles: true }}));
                
                // Chờ 500ms để React xử lý sự kiện Paste
                await new Promise(r => setTimeout(r, 500));
                ...
                if (TryParseJson(result, out var jsObj) && !jsObj.GetProperty("ok").GetBoolean())
                {
                    MessageBox.Show("Lỗi JS: " + (jsObj.TryGetProperty("error", out var e) ? e.GetString() : "Lỗi không xác định"));
                    return false;
                }
            ...
            {
                MessageBox.Show($"Lỗi C#: {ex.Message}");
                return false;
            }
        }

        private bool TryParseJson(string raw, out JsonElement element)
        {
            element = default;
