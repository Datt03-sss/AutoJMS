...
namespace AutoJMS
{
    ...
    public class WaybillTrackingService : ITrackingService
    {
        ...
        private readonly Dictionary<string, int> _columnWidthCache = new Dictionary<string, int>(StringComparer.OrdinalIgnoreCase);
        private string _currentSortColumn = string.Empty;
        private ListSortDirection _currentSortDirection = ListSortDirection.Ascending;

        private const int BatchSize = 40;
        private const int MaxConcurrency = 8;
        private const int ColumnWidthPadding = 28;
        ...
        public WaybillTrackingService(DataGridView dataGrid, UIProcessBar progressBar = null)
        {
            ...
            _exportFolder = Path.Combine(AppPaths.DownloadsDir, "Trạng thái hiện tại");
        ...
        private void SetupDataGridViewBinding()
        {
            ...
            ApplyCalculatedColumnWidths(includeRows: false);
        }
        ...
        public void LoadAllDataToGrid()
        {
            ...
            var oldRaiseEvents = _bindingSource.RaiseListChangedEvents;
            _bindingSource.RaiseListChangedEvents = false;
            try
            {
                _dataGrid.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.None;

                // Đồng bộ DataTable từ danh sách _allRows một lần sau khi đã lấy xong data.
                SyncDisplayTableFromAllRows();
                ApplyCalculatedColumnWidths(includeRows: true);

                _bindingSource.RaiseListChangedEvents = oldRaiseEvents;
        ...
        private void ClearDisplayTableSilently()
        {
            var oldRaiseEvents = _bindingSource.RaiseListChangedEvents;
            _bindingSource.RaiseListChangedEvents = false;
            try
        ...
        private void ApplyCalculatedColumnWidths(bool includeRows)
        {
            if (_dataGrid.Columns.Count == 0) return;

            var headerFont = _dataGrid.ColumnHeadersDefaultCellStyle.Font
                ?? _dataGrid.Font
                ?? Control.DefaultFont;
            var cellFont = _dataGrid.DefaultCellStyle.Font
                ?? _dataGrid.Font
                ?? Control.DefaultFont;

            foreach (DataGridViewColumn column in _dataGrid.Columns)
            {
                var dataName = string.IsNullOrWhiteSpace(column.DataPropertyName)
                    ? column.Name
                    : column.DataPropertyName;

                var width = MeasureGridText(column.HeaderText, headerFont);

                if (includeRows && _displayTable.Columns.Contains(dataName))
                {
                    foreach (DataRow row in _displayTable.Rows)
                    {
                        var text = row[dataName]?.ToString();
                        width = Math.Max(width, MeasureGridText(text, cellFont));
                    }
                }
        ...
        private static int MeasureGridText(string text, System.Drawing.Font font)
        {
        ...
        private void UpdateProgress(int percent)
        {
            ...
            {
                try { _progressBar.BeginInvoke(new Action(() => UpdateProgress(percent))); }
                catch { }
                return;
            }
        ...
        {
            if (!JmsAuthStateService.HasToken) return;
            ...
            {
                AppLogger.Error("WaybillTrackingService.SearchTrackingAsync failed", ex);
                throw;
            }
            ...
            foreach (var kvp in mapping001)
            {
                ...
                if (_rowDict.TryGetValue(originalCode, out var srcRow) && _rowDict.TryGetValue(codeWithDash, out var destRow))
                {
                    ...
                    if (string.IsNullOrEmpty(destRow.MaDoan1)) destRow.MaDoan1 = srcRow.MaDoan1 ?? "";
                    if (string.IsNullOrEmpty(destRow.MaDoan2)) destRow.MaDoan2 = srcRow.MaDoan2 ?? "";
                    if (string.IsNullOrEmpty(destRow.MaDoan3)) destRow.MaDoan3 = srcRow.MaDoan3 ?? "";
                    if (string.IsNullOrEmpty(destRow.MaDoanFull)) destRow.MaDoanFull = srcRow.MaDoanFull ?? "";
                    if (destRow.SafetyEvents.Count == 0 && srcRow.SafetyEvents.Count > 0)
                    {
                        destRow.SafetyEvents = srcRow.SafetyEvents
        ...
        private static bool TryWarnIfGoogleSheetMissing()
        {
            if (GoogleSheetService.CanUseGoogleSheetFeature()) return true;
            GoogleSheetService.NotifyGoogleSheetFeatureUse("Tra cứu dữ liệu Google Sheet");
            return false;
        }
        ...
        private void ProcessTrackingData(string waybill, WaybillData wd)
        {
            ...
            row.SafetyEvents = details
                .Select(d => new TrackingSafetyEvent
                {
                    WaybillNo = d.waybillNo ?? waybill,
                    BillCode = d.billCode ?? waybill,
                    ScanTime = d.scanTime ?? "",
                    ScanNetworkCode = d.scanNetworkCode ?? ""
                })
                .ToList();
        ...
        public List<TrackingRow> GetAllRows() => _allRows?.ToList() ?? new List<TrackingRow>();
    ...
    public class TrackingRow
    {
        ...
        public string PrintApprovalStatusName { get; set; } = "";
        public string PrintSenderNetworkCode { get; set; } = "";
        public int? PrintApprovalPrintCount { get; set; }
        public string PrintApplyStaffName { get; set; } = "";
        public List<TrackingSafetyEvent> SafetyEvents { get; set; } = new();
    }

    public class TrackingSafetyEvent
    {
        public string WaybillNo { get; set; } = "";
        public string BillCode { get; set; } = "";
        public string ScanTime { get; set; } = "";
        public string ScanNetworkCode { get; set; } = "";
    }
