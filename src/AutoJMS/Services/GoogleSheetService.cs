...
namespace AutoJMS
{
    public static class GoogleSheetService
    {
        ...
        public static void InitService()
        {
            try
            {
                bool available = Access.IsAvailableAsync().GetAwaiter().GetResult();
                if (!available)
                {
                    WarnGoogleSheetUnavailable("Thiếu token broker hoặc credential local fallback hợp lệ.");
                    return;
                }
            }
            catch (Exception ex)
            {
                WarnGoogleSheetUnavailable(ex.Message);
            }
        }
        ...
        private static string ResolveCredentialPath()
        {
            ...
            if (string.Equals(configuredPath, "service_account.json", StringComparison.OrdinalIgnoreCase) ||
                string.Equals(configuredPath.Replace('/', Path.DirectorySeparatorChar), Path.Combine("AppData", "service_account.json"), StringComparison.OrdinalIgnoreCase))
            {
                return AppPaths.GoogleServiceAccountJson;
            }

            return Path.Combine(AppPaths.InstallRoot, configuredPath);
        }

        public static bool IsConfigValid()
        {
            return IsGoogleSheetAvailable() && !string.IsNullOrWhiteSpace(DATA_SPREADSHEET_ID);
        }

        public static bool IsGoogleSheetAvailable()
        {
            try
            {
                return Access.IsAvailableAsync().GetAwaiter().GetResult();
            }
            catch
            {
                return false;
        ...
        {
            if (!IsGoogleSheetAvailable())
            {
                NotifyGoogleSheetFeatureUse("Google Sheet");
                return false;
            }
        ...
        public static async Task<List<IList<IList<object>>>> BatchReadAsync(string spreadsheetId, List<string> ranges)
        {
            ...
            try
            {
                ...
                if (rangesToFetch.Count > 0)
                {
                    ...
                    {
                        return await Access.ReadAsync(new GoogleSheetReadRequest
                        {
                    ...
                    if (response?.Success == true && response.ValueRanges != null)
                    {
                        for (int i = 0; i < response.ValueRanges.Count && i < rangesToFetch.Count; i++)
                        {
                            var fetchedData = response.ValueRanges[i] ?? new List<IList<object>>();
                    ...
                    {
                        AppLogger.Warning($"[GoogleSheets] BatchRead failed provider={response.ProviderName}: {response.Message}");
                    }
                }
        ...
        public static async Task ClearSheetAsync(string spreadsheetId, string range)
        {
            ...
            try
            {
                if (!CanUseGoogleSheetFeature()) return;
                await CallWithRetryVoidAsync(async () =>
                {
                    var result = await Access.WriteAsync(new GoogleSheetWriteRequest
                    {
                        Operation = GoogleSheetWriteOperation.Clear,
                        SpreadsheetId = spreadsheetId,
                        Range = range
                    }).ConfigureAwait(false);
        ...
        public static async Task UpdateBumpSheetAsync(IList<IList<object>> values, string spreadsheetId, string range)
        {
            ...
            try
            {
                ...
                await CallWithRetryVoidAsync(async () =>
                {
                    var result = await Access.WriteAsync(new GoogleSheetWriteRequest
                    {
                        Operation = GoogleSheetWriteOperation.Update,
                        SpreadsheetId = spreadsheetId,
                        Range = range,
        ...
        {
            if (NetworkState.Current == NetworkStatus.Offline) return new List<IList<object>>();
            if (!CanUseGoogleSheetFeature()) return new List<IList<object>>();
