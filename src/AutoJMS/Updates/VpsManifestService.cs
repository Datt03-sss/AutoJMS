...
namespace AutoJMS
{
    public class VpsManifestService
    {
        private readonly string _baseUrl;
        private readonly DataHubManifestUrls _urls;
        ...
        public DataHubManifestUrls Urls => _urls;

        public VpsManifestService(string baseUrl, DataHubManifestUrls urls)
        {
            _baseUrl = baseUrl?.TrimEnd('/') ?? "";
            _urls = urls ?? new DataHubManifestUrls();
            _http = new HttpClient(new AppHttpCaptureHandler(new HttpClientHandler(), "VpsManifestService")) { Timeout = TimeSpan.FromSeconds(15) };
        }
        ...
        private async Task<T> FetchAsync<T>(string relativePath, CancellationToken ct = default) where T : new()
        {
            ...
            {
                AppLogger.Warning($"VpsManifestService: fetch {relativePath} failed: {ex.Message}");
                return new T();
            }
        }
        ...
        public async Task<TierDefinitions> FetchTierDefinitionsAsync(CancellationToken ct = default)
        {
            ...
            {
                AppLogger.Warning($"VpsManifestService: fetch tier-definitions failed: {ex.Message}");
        ...
        public async Task<string> FetchStringAsync(string relativePath, CancellationToken ct = default)
        {
            ...
            {
                AppLogger.Warning($"VpsManifestService: fetch string {relativePath} failed: {ex.Message}");
                return null;
            }
        }
        ...
        public async Task<byte[]> FetchBytesAsync(string relativePath, CancellationToken ct = default)
        {
            ...
            {
                AppLogger.Warning($"VpsManifestService: fetch bytes {relativePath} failed: {ex.Message}");
                return null;
            }
        }
    }
